...
func Register(path string, method Method, consumes []string, produces []string,
	...
	for _, route := range routes {
		var match, _, _ = route.matchRoute(regRoute)
		if match {
...
func handler(resp http.ResponseWriter, req *http.Request) {
	...
	for _, route := range routes {
		var match, status, pathParams = route.matchRoute(reqRoute)
	...
	handleError(resp, httpStatusCode)
}

func handleError(resp http.ResponseWriter, httpStatusCode int) {
